...
def results(request):
    year = int(request.POST.get('year', 2000))
    ...
    encoding = int(request.POST.get('encoding'))
...
#         DNN = tf.keras.models.load_model(dnn)
    ...
    return render(request, "results.html", {"predictions":model_predict, "score":  r2_score, "nrmse": nrmse_score})